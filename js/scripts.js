...
document.addEventListener('DOMContentLoaded', function() {
	...
	if (mainSlider) {
		new Swiper('.main_slider .swiper', {
			...
			on: {
				init: swiper => {
					let bullets = swiper.el.querySelectorAll('.swiper-pagination-bullet')

					bullets[0].classList.remove('active')

					setTimeout(() => bullets[0].classList.add('active', 'first_animate'))
				},
				slideChangeTransitionEnd: swiper => {
					let bullets = swiper.el.querySelectorAll('.swiper-pagination-bullet')

					bullets[0].classList.remove('first_animate')
				}
			}
		})
	}
	...
	$('.product_info .mob_spoler_btn').click(function(e) {
		e.preventDefault()
	...
	cookiesAccepted === 'true'
		? $('.cookie_modal').fadeOut(200)
		: $('.cookie_modal').fadeIn(300)


	$('.cookie_modal .ok_btn').click(function(e) {
		e.preventDefault()
	...
	$('body').on('click', '.accordion .accordion_item .head', function(e) {
		...
		if (item.hasClass('active')) {
			item.removeClass('active').find('.hide').slideUp(300)
		} else {
			accordion.find('.accordion_item').removeClass('active')
			accordion.find('.hide').slideUp(300)

			item.addClass('active').find('.hide').slideDown(300)
		}
	})


	// Category spoler
	$('.category_info .spoler_btn').click(function(e) {
		e.preventDefault()
	...
	// Mob. footer
	$('footer .title a').click(function(e) {
		e.preventDefault()
